...
{if !empty($thumbnails) && ($theme_config->slick_enabled || $theme_config->photoswipe)}
  ...
  {* Plugin_Custom_Thumbnail *}
            <figure class="new-title-figure-slick">
            {assign var="thumbnail_found" value=false}
            {if !empty($CUSTOM_THUMBNAILS)}
              {foreach from=$CUSTOM_THUMBNAILS key=ext item=path}
                {if $thumbnail.file_ext == $ext}
                  <img src="{$path}" alt="{$thumbnail.TN_ALT}"
                      class="img-thumbnail img-fluid {if isset($thumbnail.path_ext)}path-ext-{$thumbnail.path_ext}{/if} {if isset($thumbnail.file_ext)}file-ext-{$thumbnail.file_ext}{/if}"
                      title="{if isset($thumbnail.TN_TITLE)}{$thumbnail.TN_TITLE}{/if}">
                  <figcaption class="bottom-caption-slick">{$thumbnail.NAME}</figcaption>
                  
                  {assign var="thumbnail_found" value=true}
                  {break}
                {/if}
              {/foreach}
            {/if}
            ...
            {if !$thumbnail_found}
              <img src="{$thumbnail.path}" alt="{$thumbnail.TN_ALT}" title="{if isset($thumbnail.TN_TITLE)}{$thumbnail.TN_TITLE}{/if}" class="img-fluid {if isset($thumbnail.path_ext)}path-ext-{$thumbnail.path_ext}{/if} {if isset($thumbnail.file_ext)}file-ext-{$thumbnail.file_ext}{/if}">
            {/if}
          </figure>
  {* Plugin_Custom_Thumbnail *}
          </a>
