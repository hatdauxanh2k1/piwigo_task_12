    ...
    <div class="card card-thumbnail {if isset($thumbnail.path_ext)}path-ext-{$thumbnail.path_ext}{/if} {if isset($thumbnail.file_ext)}file-ext-{$thumbnail.file_ext}{/if}">
      ...
      {* Plugin_Custom_Thumbnail *}
        <a href="{$thumbnail.URL}" data-index="{$idx}" class="ripple{if isset($smarty.cookies.view) and $smarty.cookies.view != 'list'} d-block{/if}">
            {assign var="thumbnail_customs" value=false}  
            {if !empty($CUSTOM_THUMBNAILS)}
                {foreach from=$CUSTOM_THUMBNAILS key=ext item=path}
                {if $thumbnail.file_ext == $ext}
                        <img class="{if isset($smarty.cookies.view) and $smarty.cookies.view == 'list'}card-img-left{else}card-img-top{/if} thumb-img" src="{$path}" alt="{$thumbnail.TN_ALT}" title="{$thumbnail.TN_TITLE}">
                    {assign var="thumbnail_customs" value=true}
                    {break}
                {/if}
                {/foreach}
            {/if}
            
            {if !$thumbnail_customs}
